...
public class WordCounter{
    ...
    private static int readWords(String file){
        try(FileReader fileReader = new FileReader(file)){ // in questo modo viene istanziato un oggetto con cui si può leggere da file 
            BufferedReader bufferReader = new BufferedReader(fileReader); // in questo modo viene istanziato un oggetto con cui si può leggere da buffer, in modo da leggere in maniera più efficiente
            String line; 
            int wordCount = 0;

            while((line = bufferReader.readLine())!= null){ // esiste quindi un metodo di default per il bufferReader che consente la lettura di una stringa
